...
bool Adafruit_INA219::setCalibration(int busVolt, float shunt, int shuntMilliVolt, float maxExpectedCurrent, String &outConfig)
{

  float maxCurrent = (shuntMilliVolt * 0.001) / shunt;
  if (maxExpectedCurrent < maxCurrent)
    maxCurrent = maxExpectedCurrent;

  float minCurrentLSB = ceil(maxCurrent * 1000000 / 32727) / 1000000;
  float maxCurrentLSB = ceil(maxCurrent * 1000000 / 4096) / 1000000;

  float currentLSB = minCurrentLSB;
  float calibration = trunc(0.04096 / (currentLSB * shunt));
  float powerLSB = currentLSB * 20;
  float maxPossibleCorrent = currentLSB * 32727;
  float maxCurrentBeforeOverflow = maxPossibleCorrent < maxExpectedCurrent ? maxPossibleCorrent : maxExpectedCurrent;
  float maxShuntVoltage = maxCurrentBeforeOverflow * shunt;
  float maxShuntVoltageBeforeOverflow=maxShuntVoltage<(shuntMilliVolt*0.001)?maxShuntVoltage:shuntMilliVolt*0.001;
  float maxPowerBeforeOverflow = maxCurrentBeforeOverflow * busVolt;

  ina219_calValue = calibration;
  ina219_currentDivider_mA = 1000 / (currentLSB*1000000); //25;    // Current LSB = 40uA per bit (1000/40 = 25)
  ina219_powerMultiplier_mW = powerLSB * 1000;  //0.8f; // Power LSB = 800uW per bit
  char buf[1024];
  sprintf(buf, "CurrentLSB: %f\nPowerLSB: %f\nBeforeOverflow =>\n\tCurrent: %f\n\tShuntVoltage: %f\n\tMaxPower: %f\n\n", currentLSB, powerLSB, maxCurrentBeforeOverflow, maxShuntVoltageBeforeOverflow, maxPowerBeforeOverflow);
  outConfig = buf;
  int shuntConf = 0;
  switch (shuntMilliVolt)
  {
  case 320:
    shuntConf = INA219_CONFIG_GAIN_8_320MV;
    break;
  case 160:
    shuntConf = INA219_CONFIG_GAIN_4_160MV;
    break;
  case 80:
    shuntConf = INA219_CONFIG_GAIN_2_80MV;
    break;
  case 40:
    shuntConf = INA219_CONFIG_GAIN_1_40MV;
    break;
  default:
    return false;
  }
  ...
  uint16_t config = (busVolt == 16 ? INA219_CONFIG_BVOLTAGERANGE_16V : INA219_CONFIG_BVOLTAGERANGE_32V) |
                    shuntConf | INA219_CONFIG_BADCRES_12BIT |
...
void Adafruit_INA219::setCalibration_test()
{
  ina219_calValue = 40960;
  ina219_currentDivider_mA = 100;   //25;    // Current LSB = 40uA per bit (1000/40 = 25)
  ina219_powerMultiplier_mW = 0.2f; //0.8f; // Power LSB = 800uW per bit
